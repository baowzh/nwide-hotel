...
class Admin extends Controller{  
    protected function _initialize(){
		...
		$arr=array();
        $side=db('Module')->where($map)->order('sort asc')->field("id,title,pid")->select();
        foreach( $side as $n=> $val ){
				$arr[]=$val['id'];
				$map2["pid"]=$val['id'];
			    $list=db('Module')->where($map2)->field("id,title,pid")->order('sort asc')->select( );
		...
		unset($map);
		$map['id']=array("in",$arr);
        $side=db('Module')->where($map)->order('sort desc')->field("id,title,pid")->select();
